<script>
    $(document).ready(function(){
        ...
        $(document).keypress(function(event){
            ...
            if(keycode == '93'){
                $.ajax({
                    url: "{{route('supplier.debit_credit.get_credit_fields')}}",
                    method: 'post',
                    data: {
                        key : key_value,
                    },
        ...
        $('.calcluate-debit-credit-values').click(function(){
            $.ajax({
                url: "{{route('supplier.debit_credit.calculate_debit_credit_values')}}",
                method: 'post',
    ...
    function checkColor(index)
    {
        ...
        $.ajax({
            url: "{{route('supplier.debit_credit.get_color')}}",
            method: 'post',
            data: {
                id : account_id,
            },
    ...
    function debitQuantity(index)
    {
        ...
        if(isNaN(qty))
        {
            ...
            $.ajax({
                ...
                success: function(response){
                    var price = response.purchasing_price;
    ...
    {
        $.ajax({
            url: "{{route('supplier.debit_credit.delete')}}",
            method: 'post',
            data: {
                id : id,
            },
    ...
    function debitCreditAccount(key,is_append)
    {
        ...
        $.ajax({
            url: "{{route('supplier.debit_credit.get_customer_vehicle')}}",
            method: 'post',
            data: {
    ...
    jQuery(document).ready(function ($) {
        $('#store-debit-credit-sale').on('click', function (event) {
            ...
            data = $('#debitCreditForm').serialize();
            $.ajax({
                url: "{{route('supplier.debit_credit.store')}}",
                method: 'post',
                data: data,
                headers: {
                    'X-CSRF-TOKEN': "{{ csrf_token() }}"
        ...
        $('#update-debit-credit-sale').on('click', function (event) {
            ...
            data = $('#debitCreditUpdateForm').serialize();
            $.ajax({
                url: "{{route('supplier.debit_credit.update_form')}}",
                method: 'post',
                data: data,
                headers: {
                    'X-CSRF-TOKEN': "{{ csrf_token() }}"
